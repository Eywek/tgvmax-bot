      ...
      <form>
        ...
        <div class="flex mb-4">
          <div class="w-1/2 pr-2">
            <div class="mb-4">
              <label class="block text-gray-700 text-sm font-bold mb-2" for="cron">Cron</label>
              <input
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none"
                id="cron"
                type="text"
                placeholder="0 0 * * TUE (Optionnal)"
                v-model="cron"
              />
            </div>
          </div>
          <div class="w-1/2 pl-2">
            <div class="mb-4">
              <label class="block text-gray-700 text-sm font-bold mb-2" for="maxTravels">Max travels</label>
              <input
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none"
                id="maxTravels"
                type="text"
                placeholder="Max at same time"
                v-model="maxTravels"
              />
            </div>
          </div>
    ...
    async submitCron () {
      const res = await fetch(`${process.env.API_URL}/crons`, {
        ...
        body: JSON.stringify({
          ...
          booker: this.booker,
          book: this.book,
          maxTravels: parseInt(this.maxTravels),
        })
      })
      const cron = await res.json()
      store.state.crons.push(cron)
      store.state.travels.push(...cron.travels)
      this.$emit('close')
    },
    close() {
      this.$emit('close')
